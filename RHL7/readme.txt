...
4. Copy daemon.py, server.py, LICENSE, NVDARemoteServer, NVDARemoteServer.1 (manual), NVDARemoteCertificate.1 (manual), NVDARemoteCertificate, NVDARemoteServer.service and server.pem into ~/rpmbuild/SOURCES/NVDARemoteServer-1.4, creating the directory if it doesn't exist
5. Navigate to the SOURCES directory, and tar the NVDARemoteServer-1.4 folder: tar -czf server.tar.gz NVDARemoteServer-1.4
